#!/usr/bin/env python3
"""Generate Boyer-backed ground truth for K_{2,3} homeomorph detection.

The oracle is the installed ``planarity`` CLI in specific-graph mode with
``-2``, which searches for a subgraph homeomorphic to ``K_{2,3}``.
...
DEFAULT_OUT_PATH = Path(__file__).with_name("k23_homeomorph_ground_truth_100k.json.gz")
...
def classify_k23(planarity_bin: str, input_path: Path, output_path: Path) -> bool:
    completed = subprocess.run(
        [planarity_bin, "-s", "-q", "-2", str(input_path), str(output_path)],
    ...
    if completed.returncode == 0:
        return False
    raise RuntimeError(
        f"planarity -2 failed on {input_path} with unexpected exit code {completed.returncode}"
    )
...
def emit_progress(current: int, total: int, start_time: float, family: str) -> None:
    ...
    sys.stderr.write(
        f"\r[k23-100k] {current:>7}/{total:<7} {percentage:6.2f}%  "
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="k23_ground_truth_") as scratch:
        scratch_dir = Path(scratch)
        input_path = scratch_dir / "current_graph.txt"
        output_path = scratch_dir / "current_witness.txt"
        ...
        with gzip.open(args.output, "wt", encoding="utf-8", compresslevel=9) as handle:
            header = {
                ...
                "generator": "edge_addition_planarity_suite_k23_mixed_family_cli",
                "primary_oracle": "k23_homeomorph_boolean",
            ...
            for index in range(args.count):
                ...
                case_name = f"{family}_{index:06d}"
                write_planarity_input(input_path, node_count, edges)
                has_k23_homeomorph = classify_k23(args.planarity_bin, input_path, output_path)
                ...
                positive_counts[family] += int(has_k23_homeomorph)
                overall_positive_count += int(has_k23_homeomorph)
    ...
    print(
        f"summary: has_k23_homeomorph={overall_positive_count}, "
        f"without_k23_homeomorph={args.count - overall_positive_count}, elapsed={elapsed:.2f}s"
    )
    print("family counts:")
    for family in FAMILY_SEQUENCE:
        print(
            f"  {family}: total={family_counts[family]} "
            f"positive={positive_counts[family]} negative={family_counts[family] - positive_counts[family]}"
        )
