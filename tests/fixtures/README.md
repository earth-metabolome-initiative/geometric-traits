...
`planarity_ground_truth_100k.json.gz` is the checked-in Boyer reference corpus
...
`k23_homeomorph_ground_truth_100k.json.gz` is the checked-in Boyer reference
corpus for `K_{2,3}` homeomorph validation against `planarity -2`.
...
`k33_homeomorph_ground_truth_100k.json.gz` is the checked-in Boyer reference
corpus for `K_{3,3}` homeomorph validation against `planarity -3`.
...
`topological_validity_ground_truth_1m_v4.json.gz` is the checked-in Boyer
reference corpus for the combined boolean validity surface:
...
stored `katz_alpha` values are chosen from a safe max-degree-based bound, so
the `NetworkX` runs converge on the full corpus. Katz scores are also
