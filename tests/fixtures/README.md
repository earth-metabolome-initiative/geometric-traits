...
`k23_homeomorph_ground_truth_100k.json.gz` is the local-only Boyer reference
...
```bash
python3 tests/fixtures/generate_k23_homeomorph_ground_truth.py \
  --planarity-bin /path/to/planarity-4.x
```

It is consumed by `tests/test_k23_homeomorph_local_corpus.rs`.

# K33 Homeomorph Ground-Truth Fixtures

`k33_homeomorph_ground_truth_100k.json.gz` is the local-only Boyer reference
...
```bash
python3 tests/fixtures/generate_k33_homeomorph_ground_truth.py \
  --planarity-bin /path/to/planarity-4.x
```

It is consumed by `tests/test_k33_homeomorph_local_corpus.rs`.

# Combined Topological Validity Ground-Truth Fixtures

`topological_validity_ground_truth_1m_v4.json.gz` is the local-only Boyer
...
```bash
python3 tests/fixtures/generate_topological_validity_ground_truth.py \
  --planarity-bin /path/to/planarity-4.x
```
